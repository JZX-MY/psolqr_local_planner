...
namespace psolqr_planner
{
   ...
   */
  PSO::PSO(int nx, int ny, double resolution, double origin_x, double origin_y,int n_particles,int n_inherited,
           int pointNum , double w_inertial, double w_social, double w_cognitive, double obs_factor ,int max_speed, 
           double state_init_ratio, double disturbance_state_init_ratio, bool pub_particles,int max_iter)
    : nx_(nx)
    , ny_(ny)
    , ns_(nx*ny)
    ...
    , state_init_ratio_(state_init_ratio)
    , disturbance_state_init_ratio_(disturbance_state_init_ratio)
   ...
   */
  bool PSO::plan(const unsigned char* global_costmap, std::vector< std::pair<int, int>>& path_g, std::vector< std::pair<int, int>>& path, double origin_x, double origin_y)
  {
    ...
    // Set local planning starting point and target point
    ...
    path_g.erase(path_g.begin());
    path_g.pop_back();
    generateInitialPositions(initialPositions,start_d,goal_d,path_g);
    
    std::cout<<"PSO:  Successfully generated initial position of particle swarm"<<std::endl;

    //Determine whether the current inheritance is possible
    bool Allow_inheritance = false;
    if(inherited_particles_.size()==n_inherited_)
    {
      size_t inherited_pointNum= inherited_particles_[0].personal_best_pos.size();
      if(inherited_pointNum == pointNum_)
      {
        Allow_inheritance = true;
      } 

      //ROS_INFO("inherited pointNum: %zu   current pointNum: %d ", inherited_pointNum, pointNum_);
    }
    ...
    for (int i = 0; i < n_particles_; ++i)
    {
      ...
      if ((i<n_inherited_)&&(Allow_inheritance))
      {
  ...
  void PSO::generateInitialPositions(PositionSequence &initialPositions,const std::pair<double, double> start_d,const std::pair<double, double> goal_d, const std::vector< std::pair<int, int>> Keypoints)
  {
      // Use a random device and engine to generate random numbers
      std::random_device rd;
      std::mt19937 gen(rd());
      pointNum_=Keypoints.size();
      ...
      //Calculate strategy switching boundaries
      int particles_s,particles_sr;
      particles_sr=disturbance_state_init_ratio_*n_particles_;
      particles_s=particles_sr+state_init_ratio_*n_particles_;
      ...
      for (int i = 0; i < n_particles_; ++i)
      {
          ...
          if(i<particles_sr)
          {
            ...
            {
                double random_offset_x = dist(gen) * max_speed_;
                double random_offset_y = dist(gen) * max_speed_;

                int new_x = point.first + static_cast<int>(random_offset_x);
                int new_y = point.second + static_cast<int>(random_offset_y);

                particlePositions.emplace_back(new_x, new_y);
            }

            initialPositions.push_back(particlePositions);
          }
          else if(i<particles_s)
          {
            initialPositions.push_back(Keypoints);
          }
          else
          {
  ...
  void PSO::publishParticleMarkers(const std::vector<std::pair<int, int>>& positions, const int& index) 
  {
    ...
    for (const auto& position : positions) {
      ...
      //ROS_INFO("resolution: %f, position.first: %d, origin_x: %f, p.x: %f, position.second: %d, origin_y: %f, p.y: %f", resolution_, position.first, origin_x_, p.x, position.second, origin_y_, p.y);
